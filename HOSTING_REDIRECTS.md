...
The language variant for legacy aliases uses English as the documented default because no supported legacy language signal exists. Do not create redirect chains. Unknown URLs must return the generated `dist/404.html` with HTTP 404. All non-root canonical routes are slash-free; a slash-suffixed known route gets one 301 to its slash-free form.

## Root behavior

`/` returns HTTP 200 and serves `dist/index.html`, whose prerendered canonical is
`https://www.elitemaisonmarketing.com/en`. The existing React root behavior may
